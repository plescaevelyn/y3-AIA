...
[allcombinations3,partial_combinations3,combinations3] = find3combinations(na); % finding all the combinations of na and nb
...
switch m
    ...
    for k = 1:length(s.id.InputData) 
        ...
        for i = 1:length(allcombinations2)
            if (k>i && k > allcombinations2(i,1) && k > allcombinations2(i,2))
                % combinatiile de tip y(k-i)u(k-j)
                phi_id(k,i+1+2*na+length(combinations2)) = s.id.OutputData(k-allcombinations2(i,1))*s.id.InputData(k-allcombinations2(i,2));
            end
        end
    ...
    for k = 1:length(s.id.InputData) 
        ...
        for i = 1:length(combinations2)
            if k>i && k > combinations2(i,1) && k > combinations2(i,2)
                ...
                phi_id(k,i+2*na+1) = s.id.OutputData(k-combinations2(i,1))*s.id.OutputData(k-combinations2(i,2));
                % combinatiile de tip u(k-i)u(k-j)
                phi_id(k,i+1+2*na+length(allcombinations2)+length(allcombinations2)) = s.id.InputData(k-combinations2(i,1))*s.id.InputData(k-combinations2(i,2));
            end
        end
        ...
        for i = 1:length(allcombinations2)
            if (k>i && k > allcombinations2(i,1) && k > allcombinations2(i,2))
                % combinatiile de tip y(k-i)u(k-j)
                phi_id(k,i+1+2*na+length(allcombinations2)) = s.id.OutputData(k-allcombinations2(i,1))*s.id.InputData(k-allcombinations2(i,2));
            end
        end

        for i = 1*na+2*length(allcombinations2)+length(allcombinations2)+1:2*na+2*length(allcombinations2)+length(allcombinations2)
            phi_id(k,i+1) = s.id.OutputData(i)^2; % y(k-i)^m (valoarea maxima)
        end

        for i = 2*na+2*(length(allcombinations2)/2)+length(allcombinations2)+1:3*na+2*length(allcombinations2)+length(allcombinations2)
            phi_id(k,i+1) = s.id.InputData(i)^2; % u(k-i)^m
        end 

        for i = 1:length(allcombinations3)
            if (k>i && k > allcombinations3(i,2) && k > allcombinations3(i,1) && k > allcombinations3(i,3) && k > allcombinations3(i,2))
                ...
                phi_id(k,i+3*na+2*length(allcombinations2)+length(allcombinations2)) = s.id.OutputData(k-allcombinations3(i,1))*s.id.OutputData(k-allcombinations3(i,2))*s.id.InputData(k-allcombinations3(i,3));
                % combinatiile de tip y(k-i)u(k-i)u(k-j)
                phi_id(k,i+3*na+2*length(allcombinations2)+length(allcombinations2)+length(allcombinations3)) = s.id.InputData(k-allcombinations3(i,1))*s.id.InputData(k-allcombinations3(i,2))*s.id.OutputData(k-allcombinations3(i,3));
            end
        end

        for i = 3*na+2*length(allcombinations2)+length(allcombinations2)+1:4*na+2*length(allcombinations3)
            phi_id(k,i+1) = s.id.OutputData(i)^3; % y(k-i)^m (valoarea maxima)
        end

        for i = 4*na+2*(length(allcombinations2))+length(allcombinations2)+2*length(allcombinations3)+1:5*na+2*(length(allcombinations2))+length(allcombinations2)+2*length(allcombinations3)
...
switch m
    ...
    for k = 1:length(s.val.InputData) 
        ...
        for i = 1:length(allcombinations3)
            if (k>i && k > allcombinations3(i,2) && k > allcombinations3(i,1) && k > allcombinations3(i,3) && k > allcombinations3(i,2))
                % combinatiile de tip y(k-i)y(k-i)u(k-j)
                phi_val(k,i+3*na+2*length(allcombinations2)+length(allcombinations2)) = s.val.OutputData(k-allcombinations3(i,1))*s.val.OutputData(k-allcombinations3(i,2))*s.val.InputData(k-allcombinations3(i,3));
                % combinatiile de tip y(k-i)u(k-i)u(k-j)
                phi_val(k,i+3*na+2*length(allcombinations2)+length(allcombinations2)+1) = s.val.InputData(k-allcombinations3(i,1))*s.val.InputData(k-allcombinations3(i,2))*s.val.OutputData(k-allcombinations3(i,3));
            end
        end

        for i = 3*na+2*length(allcombinations2)+length(allcombinations2)+1:4*na+2*length(allcombinations3)
            phi_val(k,i+1) = s.val.OutputData(i)^3; % y(k-i)^m (valoarea maxima)
        end
    ...
    for k = 1:length(s.id.InputData) 
        ...
        for i = 1:length(combinations2)
            if k>i && k > combinations2(i,1) && k > combinations2(i,2)
                ...
                % combinatiile de tip u(k-i)u(k-j)
                phi(k,i+1+2*na+length(allcombinations2)+length(allcombinations2)) = s.val.InputData(k-combinations2(i,1))*s.val.InputData(k-combinations2(i,2));
            end
        end
        ...
        for i = 1:length(allcombinations2)
            if (k>i && k > allcombinations2(i,1) && k > allcombinations2(i,2))
                % combinatiile de tip y(k-i)u(k-j)
                phi(k,i+1+2*na+length(allcombinations2)) = ysim(k-allcombinations2(i,1))*s.val.InputData(k-allcombinations2(i,2));
            end
        end
        ...
        for i = 1:length(allcombinations3)
            if (k>i && k > allcombinations3(i,2) && k > allcombinations3(i,1) && k > allcombinations3(i,3) && k > allcombinations3(i,2))
                % combinatiile de tip y(k-i)y(k-i)u(k-j)
                phi(k,i+3*na+2*length(allcombinations2)+length(allcombinations2)) = ysim(k-allcombinations3(i,1))*ysim(k-allcombinations3(i,2))*s.val.InputData(k-allcombinations3(i,3));
                % combinatiile de tip y(k-i)u(k-i)u(k-j)
                phi(k,i+3*na+2*length(allcombinations2)+length(allcombinations2)+1) = s.val.InputData(k-allcombinations3(i,1))*s.val.InputData(k-allcombinations3(i,2))*ysim(k-allcombinations3(i,3));
            end
        end

        for i = 3*na+2*length(allcombinations2)+length(allcombinations2)+1:4*na+2*length(allcombinations3)
            phi(k,i+1) = ysim(i)^3; % y(k-i)^m (valoarea maxima)
        end

        for i = 4*na+2*(length(allcombinations2))+length(allcombinations2)+2*length(allcombinations3)+1:5*na+2*(length(allcombinations2))+length(allcombinations2)+2*length(allcombinations3)
            phi(k,i+1) = s.val.InputData(i)^3; % u(k-i)^m
        end 
